...
public class RssfeedsActivity extends AppCompatActivity {
	...
	public void openRssfeed(RssfeedLoader loader, boolean markAsViewedNow) {
		...
		if (fragmentItems != null && fragmentItems.isAdded()) {
			...
			if (loader.hasError()) {
				SnackbarManager.show(Snackbar.with(this).text(R.string.rss_error).colorResource(R.color.colorAccent));
				return;
			}
			if (loader.getChannel() == null || loader.getChannel().getItems().size() == 0) {
				SnackbarManager.show(Snackbar.with(this).text(R.string.rss_notloaded).colorResource(R.color.colorAccent));
				return;
			}
